...
Let's break it down:
* docker run: we know that, it downloads the required images, create and run a container
...
* node:6.0: this is the image and tag of the image. We'll be using node 6.0
...
Once this runs, you can access the server at [localhost:8000](localhost:8000) or your-IP:8000. If you are unsure what "your-IP" is, you can just try the following:
```
...
As you can see, we added one dependency: `require('redis')` node module. Because of this, you'll need to install the NPM dependency. If you are unsure why, you can just grab the package.json file and node_modules folder from the step3-adding-redis in the repo.
...
To fix this, let's first retrieve your Mac's IP address:
```
...
As before, you can access the server at [localhost:8000](localhost:8000) or your-IP:8000. If you are unsure what "your-IP" is, you can just try the following:
```
open http://$(ipconfig getifaddr en0):8000
```

Let's open a little aside on networking and all this works. To get a better feel, you can execute the following command:
```
docker run --rm --add-host="redis:${REDIS_IP}" node:6.0 cat /etc/hosts
```
...
1. We'll be using the [node:6.0 image](https://hub.docker.com/_/node/) which comes with node pre-installed (who would have thought!)
...
Like before, you can test at [localhost:8000](localhost:8000) or by running:
```
open http://$(ipconfig getifaddr en0):8000
```
...
As with every other step, access the application at [localhost:8000](localhost:8000) or by running:
```
open http://$(ipconfig getifaddr en0):8000
```
